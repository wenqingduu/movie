...
4. 还需要抽取 characters。每个 character 需要包含：
   ...
   - asset_prompt，用于生成人物身份参考图和 3D 人脸建模的提示词，必须强调正脸、清晰五官、无遮挡、单人、简洁背景
...
5. 人物参考图后续会用于 FaceLift 单图 3D 人脸建模和 InsightFace 身份特征提取，因此 prompt 必须优先保证脸部可用：
   - 单人，正脸 frontal face，直视镜头 looking at camera。
   - 头肩像或胸像 head-and-shoulders portrait / bust portrait。
   - 五官清晰 sharp facial features，脸部占画面较大。
   - 不戴帽子、不戴口罩、不戴墨镜，头发不要遮挡眼睛和脸。
   - 背景简单干净，不要出现其他人物。
   - 保留角色的年龄、性别、发型、服装和关键外观特征。
   - 不要使用背影、侧脸、远景、动作镜头、电影剧照式复杂构图。
6. 场景背景图 prompt 应该短而清晰，描述地点、光照、天气、时代感和氛围；不要把整段剧情塞进 prompt，避免图像模型截断。
7. 所有工具调用完成后，输出一个 JSON 总结，不要输出 Markdown，不要解释。
