...
def _memory_efficient_attention(query, key, value, attention_mask=None):
    if xformers is not None and os.getenv("FACELIFT_USE_XFORMERS", "0") == "1":
        try:
            return xformers.ops.memory_efficient_attention(query, key, value, attn_bias=attention_mask)
        except Exception as exc:
            print(f"xformers attention failed; falling back to PyTorch SDPA: {exc}")
...
class XFormersMVAttnProcessor:
    ...
    def __call__(
        ...
        hidden_states = _memory_efficient_attention(query, key, value, attention_mask)
        hidden_states = attn.batch_to_head_dim(hidden_states)
